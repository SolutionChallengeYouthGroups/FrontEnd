            ...
            <PopoverContent>
                <PopoverHeader>
                    <HStack>
                        ...
                        <IconButton icon={<AddIcon/>} colorScheme="green" aria-label="add meeting time"
                        size="sm" onClick={(e) => {
                            setTimes([...group.meetingTimes, meetingTimeDefault()])
                        }}/>
                    </HStack>
                </PopoverHeader>
                <PopoverArrow/>
                <PopoverCloseButton/>
                <PopoverBody>
                    {group.meetingTimes.length === 0 ? <Text>No meeting times have been created</Text> : 
                    <VStack>
                        {group.meetingTimes.map((mt, i) => 
                            <EditableMeetingTime key={mt.name+"\t"+i} meetingTimes={group.meetingTimes} // \t is to help ensure uniqueness
                            index={i} meetingTimesChangedCallback={(ts) => setTimes(ts)}/>
                        )}
                    </VStack>}
                </PopoverBody>
            </PopoverContent>
                ...
                <ModalBody>
                    <VStack alignItems="normal">
                        {sorted.map((dayTimes, index) => 
                            dayTimes.length === 0 ? <React.Fragment key={index}/> : 
