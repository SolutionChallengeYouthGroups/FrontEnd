...
const GroupLocation = ({group, edit, originalLocation}: Props) => {
    const [location, setLocationState] = useState(group.location ? 
        GeoPointLocation.fromGeoPoint(group.location) : null);
    ...
    if (!!edit){
        return <VStack w="100%" h="100%"><GoogleMapReact onClick={e => setLocation(new GeoPointLocation(e.lat, e.lng))}
        bootstrapURLKeys={{
            key: "AIzaSyCnQIAuAR3oMVYlYm_ahxLh5B89PT3YYwg",
        }}
        center={(location ? location : defaultGeoPointLocation()).toMapCoords()}
        defaultZoom={14}>
            { originalLocation && group.location ? <MapMarker color="red" lat={originalLocation.lat} lng={originalLocation.lon}
            onContextMenu={(e) => {
                setLocation(null);
            }}/>
            : <></> }
        ...
        {originalLocation ? <Text>The red marker is the old location.
            Right click it to remove the group location.</Text>
            : <></>}
        </VStack>
    }
    if (!location){
    ...
    defaultZoom={14}>
        <MapMarker popup={<Text fontWeight="semibold">{group.name}</Text>}
