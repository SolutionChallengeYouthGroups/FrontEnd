...
const EditableMeetingTime = ({
    meetingTimes,
    index,
    meetingTimesChangedCallback,
    ...rest
}: Props) => {
    ...
    function deleteTime() {
        ...
        meetingTimesChangedCallback([
            ...meetingTimes.slice(0, index),
            ...meetingTimes.slice(index + 1),
        ]);
    }
    return (
        <VStack
        ...
        >
            ...
            <HStack width="100%">
                <Select
                    value={splitTime(meetingTime.startTime)[0]}
                    size="xs"
                    variant="flushed"
                    onChange={(e) =>
                        setMeetingProperty({
                            startTime:
                                ...
                                splitTime(meetingTime.startTime)[1],
                        })
                    }
                >
                    {_.range(0, 24).map((h) => (
                ...
                <Text>:</Text>
                <Select
                    value={splitTime(meetingTime.startTime)[1]}
                    size="xs"
                    variant="flushed"
                    onChange={(e) =>
                        setMeetingProperty({
                            startTime:
                                splitTime(meetingTime.startTime)[0] * 60 +
                ...
                <Text>-</Text>
                <Select
                    value={splitTime(meetingTime.endTime)[0]}
                    size="xs"
                    variant="flushed"
                    onChange={(e) =>
                        setMeetingProperty({
                            endTime:
                                Number.parseInt(e.target.value) * 60 +
                                splitTime(meetingTime.endTime)[1],
                        })
                    }
                >
                    {_.range(0, 24).map((h) => (
                ...
                <Text>:</Text>
                <Select
                    value={splitTime(meetingTime.endTime)[1]}
                    size="xs"
                    variant="flushed"
                    onChange={(e) =>
                        setMeetingProperty({
                            endTime:
                                splitTime(meetingTime.endTime)[0] * 60 +
                                Number.parseInt(e.target.value),
                        })
                    }
                >
